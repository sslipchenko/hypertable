...
gen_test_data() {
    if [ ! -s golden_dump.$MAX_KEYS.md5 ] ; then
        ...
        $DIGEST < golden_dump.txt > golden_dump.$MAX_KEYS.md5
        \rm -f golden_dump.txt
    fi
}

dump_keys() {
    $HT_HOME/bin/ht shell -l error --batch < $SCRIPT_DIR/dump-test-table.hql > $1.txt
    if [ $? != 0 ] ; then
    ...
    if [ $? != 0 ] ; then
        ...
        $HT_HOME/bin/ht shell -l error --batch < $SCRIPT_DIR/dump-test-table.hql > $1.again.txt
        exec 1>&-
        sleep 86400
    fi
}
