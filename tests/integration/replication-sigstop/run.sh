...
# kill local slave 1.2 with sigstop
pid=`cat repslave12.pid` 
echo "sending sigSTOP to $pid" 
kill -STOP $pid


# wait a bit till the replication finishes and till the hyperspace lock
# of the slave times out
sleep 60
...
grep "Replication.Slave repslave12 lost its hyperspace lock" repmaster1.log
if [ $? -ne "0" ];
then
  echo "grep failed"
  exit 1
