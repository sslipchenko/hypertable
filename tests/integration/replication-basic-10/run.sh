...
$ABC_HOME/bin/Replication.Slave --config=$SCRIPT_DIR/hypertable.cfg \
    ...
    --induce-failure=Replication.Slave-Update-2:throw:0 \
...
grep "induced failure code" repslave21.log
if [ $? -ne "0" ];
then
...
# and make sure there's a corrupt fragment in the 'errors' directory 
e=`ls "$HT_HOME/fs/local/hypertable/servers/replicator/errors" | wc -l`
if [ $e -ne "1" ];
then
    echo "Did not find a fragment in HT_HOME/fs/local/hypertable/servers/replicator/errors"
    exit 1
fi
