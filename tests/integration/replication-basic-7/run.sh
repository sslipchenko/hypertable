...
$ABC_HOME/bin/Replication.Master --config=$SCRIPT_DIR/hypertable.cfg \
    --induce-failure=Replication.Master-FragmentFinished-1:exit:0 \
    &> repmaster1.log &
if [ $? -ne "0" ];
then
...
echo "starting failover master"
$ABC_HOME/bin/Replication.Master --config=$SCRIPT_DIR/hypertable.cfg \
    --Hypertable.Replication.Master.Port=38099 \
    &> repmaster3.log &
if [ $? -ne "0" ];
then
  echo "Replication.Master 3 failed to start"
  exit 1
fi
...
grep "induced failure code" repmaster1.log
if [ $? -ne "0" ];
then
