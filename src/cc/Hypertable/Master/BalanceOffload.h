...
#ifndef HYPERTABLE_BALANCEOFFLOAD_H
#define HYPERTABLE_BALANCEOFFLOAD_H
...
namespace Hypertable {
  ...
  // remaining servers
  class BalanceOffload {

    public:
      BalanceOffload(ContextPtr &context) : m_context(context) { }

      void compute_plan(std::vector<RangeServerStatistics> &range_server_stats,
                        std::set<String> &offload_servers,
                        const String &root_location, BalancePlanPtr &plan);
    private:
...
#endif // HYPERTABLE_BALANCEOFFLOAD_H
