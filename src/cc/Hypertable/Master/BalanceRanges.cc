...
void BalanceRanges::compute_plan(vector<RangeServerStatistics> &stats,
                BalancePlanPtr &balance_plan) {
  clear();
  m_num_servers = stats.size();
  if (m_num_servers <= 1)
    return;
  compute_range_distribution(stats);
...
void BalanceRanges::compute_moves(BalancePlanPtr &plan) {
  ScanSpec scan_spec;
  Cell cell;
...
bool BalanceRanges::maybe_add_to_plan(const char *table,
...
void BalanceRanges::compute_range_distribution(vector<RangeServerStatistics> &stats) {
  m_num_ranges = 0;

  foreach_ht (RangeServerStatistics &rs, stats) {
