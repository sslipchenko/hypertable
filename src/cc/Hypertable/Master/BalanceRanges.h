...
#ifndef HYPERTABLE_BALANCERANGES_H
#define HYPERTABLE_BALANCERANGES_H
...
namespace Hypertable {
  ...
  class BalanceRanges {
    ...
    public:
      BalanceRanges(ContextPtr &context)
        : m_context(context), m_num_servers(0), m_num_ranges(0) { }

      void compute_plan(std::vector<RangeServerStatistics> &stats,
                        BalancePlanPtr &plan);
    ...
    private:
      ...
      bool maybe_add_to_plan(const char *table, const char *src_server,
                    ...
                    BalancePlanPtr &plan);
      void compute_range_distribution(vector<RangeServerStatistics> &stats);
...
#endif // HYPERTABLE_BALANCERANGES_H
