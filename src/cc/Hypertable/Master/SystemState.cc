...
void SystemState::display(std::ostream &os) {
  ...
  for (int i=0; i<SystemVariable::COUNT; i++) {
    if (!first)
      os << ",";
    os << SystemVariable::code_to_string(m_admin_specified[i].code) << "=" 
       << (m_admin_specified[i].value ? "true" : "false");
    first = false;
  }
  first = true;
  os << "} auto{";
  HT_ASSERT(m_auto_specified.size() == SystemVariable::COUNT);
  for (int i=0; i<SystemVariable::COUNT; i++) {
    if (!first)
      os << ",";
    os << SystemVariable::code_to_string(m_auto_specified[i].code) << "=" 
       << (m_auto_specified[i].value ? "true" : "false");
    first = false;
  }
  os << "} ";
}
