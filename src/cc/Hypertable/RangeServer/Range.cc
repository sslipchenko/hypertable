...
void Range::relinquish() {
  ...
  try {
    int64_t hash_code = MetaLog::EntityTaskAcknowledgeRelinquish::hash_code(m_metalog_entity->table, m_metalog_entity->spec);
    switch (m_metalog_entity->state.state) {
    case (RangeState::STEADY):
      if (Global::on_work_queue(hash_code)) {
        HT_WARNF("Aborting relinquish of %s becuase outstanding relinquish_acknowledge()", m_name.c_str());
        return;
      }
...
void Range::relinquish_compact_and_finish() {
  ...
  // Add tasks to work queue
  {
    ...
    Global::work_queue.push_back(acknowledge_relinquish_task);
  }
