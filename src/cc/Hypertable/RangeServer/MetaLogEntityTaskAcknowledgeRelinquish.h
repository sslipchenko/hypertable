...
namespace Hypertable {
  namespace MetaLog {
    ...
    public:
      ...
      static int64_t hash_code(const TableIdentifier &table, const RangeSpec &range_spec) {
        return md5_hash(table.id) ^ md5_hash(range_spec.start_row) ^ md5_hash(range_spec.end_row);
      }
