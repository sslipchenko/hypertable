 ...
 *
 */
void RequestHandlerDumpPseudoTable::run() {
  ...
  try {
    ...
    pseudo_table_name = Serialization::decode_vstr(&decode_ptr, &decode_remain);
    outfile = Serialization::decode_vstr(&decode_ptr, &decode_remain);
    m_range_server->dump_pseudo_table(&cb, &table, pseudo_table_name, outfile);
  }
  catch (Exception &e) {
